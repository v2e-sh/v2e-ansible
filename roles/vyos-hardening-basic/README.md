...
those belong in `vyos-hardening-v2espec`.
...
- `system config-management commit-confirm action reload` — makes confirmed-commit
  non-disruptive (reload, not reboot), so the later `vyos-hardening-v2espec` can
  apply risky firewall changes safely.
- pre-login banner.
...
listen-address, ciphers, dynamic-protection) must go in `vyos-hardening-v2espec`
with commit-confirm.**

## Run (on demand, from control)

```bash
ansible-playbook site.yml --tags vyos
```
